...
class ReverieServer: 
  ...
  def broadcast_announcement(self, announcement):
    """
    Broadcasts an announcement to all personas in the simulation.

    INPUT
      announcement: String containing the announcement message
    OUTPUT
      None
    """
    for persona_name, persona in self.personas.items():
        persona.receive_announcement(announcement)
    print(f"Announcement broadcasted: {announcement}")
    ...
    while (True): 
      ...
      if check_if_file_exists(curr_env_file):
        ...
        if env_retrieved: 
          ...
          directory = os.path.dirname(curr_move_file)
          # 디렉토리가 존재하지 않으면 생성
  ...
  def open_server(self): 
    ...
    while True: 
      ...
      try: 
        ...
        elif sim_command.lower().startswith("announce:"):
        # Broadcasts an announcement to all personas# Example: announce: There will be a town hall meeting at 3 PM today
            announcement = sim_command[9:].strip()# Remove "announce: " from the start
