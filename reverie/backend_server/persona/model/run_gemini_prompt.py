...
def run_gpt_prompt_daily_plan(persona, 
  ...
  param = { "max_tokens": 50, "temperature": 1, "top_p": 1, 
  ...
  output = safe_generate_response(prompt, param, 5, fail_safe,
                                   __func_validate, __func_clean_up)
  output = ([f"wake up and complete the morning routine at {wake_up_hour}:00 am"]
              + output)

  if debug or verbose: 
    print_run_prompts(prompt_template, persona, param, 
                      prompt_input, prompt, output)
    
  return output, [output, prompt, param, prompt_input, fail_safe]
