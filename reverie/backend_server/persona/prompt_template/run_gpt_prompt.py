...
def validate_and_convert_json(input_string):
    # 입력 문자열 앞뒤의 공백 제거
    ...
    try:
        ...
        return json.dumps(json_object)  # 유효한 JSON이면 그대로 반환
    except json.JSONDecodeError:
        pass  # JSON 형식이 아니면 다음 단계로

    # 마크다운 JSON 형식 확인
    markdown_pattern = r'```json\s*(.*?)\s*```'
    match = re.search(markdown_pattern, input_string, re.DOTALL)
    if match:
        json_content = match.group(1)
        try:
            json_object = json.loads(json_content)
            return json.dumps(json_object)  # 유효한 JSON이면 변환하여 반환
        except json.JSONDecodeError:
            pass  # 마크다운 내부의 내용이 유효한 JSON이 아니면 다음 단계로
    ...
    if match:
        ...
        try:
            ...
            pass  # JSON 객체 패턴이지만 유효한 JSON이 아니면 다음 단계로

    # 모든 검증 단계를 통과하지 못한 경우 오류 발생
    raise ValueError("입력이 유효한 JSON 형식이 아닙니다.")
...
def run_gpt_prompt_wake_up_hour(persona, test_input=None, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
      # Expecting gpt_response to be in the format: {"time": "HH"}
      try: 
        json_match = re.search(r"```json\s(.*?)\s```", gpt_response, re.DOTALL).group(1)
    
        response_dict = json.loads(json_match.strip())
        print(int(response_dict["time"]))
        return int(response_dict["time"])
  ...
  def __func_validate(gpt_response, prompt=""):
      try: 
          json_match = re.search(r"```json\s(.*?)\s```", gpt_response, re.DOTALL).group(1)
          print (gpt_response,json_match)
          response_dict = json.loads(json_match.strip())
          print(response_dict)

          hour = int(response_dict["time"])
          if 0 <= hour <= 23:
              return True
      except Exception as e:
          print("validate wakeup \n", e)
          print(gpt_response)
          return False
      return False
  ...
  # JSON 출력 형식을 프롬프트에 추가
  json_format = '{\n    "time" : "HH"\n}'
  prompt = generate_prompt(prompt_input, prompt_template)
  prompt += f"\n\nPlease provide the wake-up time in the following JSON format:\n{json_format}\nReplace HH with the two-digit hour (00-23)."
  print('디버그: wake up\n', prompt)
...
def run_gpt_prompt_daily_plan(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      json_match = validate_and_convert_json(gpt_response)
    
      response_dict = json.loads(json_match.strip())
      daily_plan = response_dict.get("daily_plan", [])
      cleaned_plan = [action.strip() for action in daily_plan if action.strip()]
      return cleaned_plan
    except :
      pass
  ...
  def __func_validate(gpt_response, prompt=""):
    try:
      ...
      response_dict = json.loads(json_match.strip())
      daily_plan = response_dict.get("daily_plan", [])
      return isinstance(daily_plan, list) and len(daily_plan) > 0
    
    except Exception as e :
      print('DEBUG:daily plan\n',e,'-',gpt_response,'-')
      return False
  ...
  prompt += f"\n\nPlease provide the daily plan in the following JSON format, adjusting the times and activities to fit the persona's schedule and lifestyle:\n{json_format}\nEach action should be a string without any punctuation at the end."
...
def run_gpt_prompt_generate_hourly_schedule(persona, 
  ...
  def create_prompt_input(persona, 
    ...
    prompt_ending = f"""
Finally, based on the information above, predict what Isabella will be doing at {curr_hour_str} in JSON format.
...
"""
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      response_dict = json.loads(json_match.strip())
      cleaned_plan = response_dict["action"]
      return cleaned_plan
    except Exception as e:
      print("DEBUG cleanup : ", gpt_response)
      raise e


  def __func_validate(gpt_response, prompt=""): 
    print('DEBUG:hourly schedule\n', gpt_response,)
    try: __func_clean_up(gpt_response, prompt=prompt)
    except: return False
    return True
...
def run_gpt_prompt_task_decomp(persona, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      json_match = validate_and_convert_json(gpt_response)
      response_dict = json.loads(json_match.strip())
      schedule = response_dict.get("schedule",[])
    
      cr = []  # 최종 출력 리스트
      for item in schedule:
          task = item["task"]
          duration = int(item["duration"])  # duration을 정수로 변환
          cr.append([task, duration])  # 원하는 형식으로 리스트에 추가
  ...
  def __func_validate(gpt_response, prompt=""): 
    ...
    print(validate_and_convert_json(gpt_response))
    print(__func_clean_up(gpt_response))
    try: 
...
def run_gpt_prompt_action_sector(action_description, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      return response_dict["sector"]
    except Exception as e:
      print(f'DEBUG: sector_cleanup:\n {e}\n{json_match}\nEND')
      raise e
...
def run_gpt_prompt_action_game_object(action_description, 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      return response_dict["object"]
    except Exception as e:
      print(f'DEBUG: action_object cleanup:\n {e}\n{json_match}\nEND')
      raise e
  ...
  def __func_validate(gpt_response, prompt=""): 
    try:
      ...
      print("DEBUG: action_object validate")
      print(e)
...
def run_gpt_prompt_pronunciatio(action_description, persona, verbose=False): 
  ...
  print ("DEBUG pronunciatio") ########
  gpt_param = { "max_tokens": 1500, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": None}
  ...
  fail_safe = get_fail_safe()
  output = Gemini_safe_generate_response(prompt, example_output, special_instruction, 3, fail_safe,
...
def run_gpt_prompt_event_triple(action_description, persona, verbose=False): 
  ...
  def __func_clean_up(gpt_response, prompt=""):
    try:
      ...
      return [response_dict["predicate"], response_dict["object"]]
    except Exception as e:
      print(f'DEBUG: event_triple cleanup:\n {e}\n{json_match}\nEND')
      raise e
  ...
  def __func_validate(gpt_response, prompt=""): 
    try:
      ...
      print("DEBUG: event_triple validate")
      print(e)
  ...
  print('DEBUG: event_triple: action_description\n', action_description ,'END')
  gpt_param = { "max_tokens": 300, 
               "temperature": 0, "top_p": 1, "stream": False,
               "frequency_penalty": 0, "presence_penalty": 0, "stop": ["\n"]}
...
def run_gpt_generate_safety_score(persona, comment, test_input=None, verbose=False): 
  ...
  print (prompt)
  fail_safe = get_fail_safe() 
  output = Gemini_safe_generate_response(prompt, 3, fail_safe,
...
def run_gpt_generate_iterative_chat_utt(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input=None, verbose=False): 
  ...
  print (prompt)
  fail_safe = get_fail_safe() 
  output = Gemini_safe_generate_response_OLD(prompt, 3, fail_safe,
                        __chat_func_validate, __chat_func_clean_up, verbose)
  print (output)
