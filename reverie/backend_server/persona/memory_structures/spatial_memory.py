...
class MemoryTree: 
  ...
  def print_tree(self): 
    def _print_tree(tree, depth):
      dash = " >" * depth
      curr_sector = curr_sector.strip()  # 문자열 앞뒤의 공백을 제거
  ...
  def get_str_accessible_sector_arenas(self, sector): 
    ...
    INPUT
      sector: A string in the format "world:sector"
    OUTPUT 
      A summary string of all the arenas that the persona can access. 
    EXAMPLE STR OUTPUT
      "bedroom, kitchen, dining room, office, bathroom"
    ...
    try:
        ...
        raise ValueError(f"Invalid sector format: {sector}. Expected format is 'world:sector'.")
    
    if not curr_sector.strip():  # 공백 처리
        return ""

    curr_world = curr_world.strip()
    curr_sector = curr_sector.strip()

    if curr_world not in self.tree:
        raise KeyError(f"World '{curr_world}' not found in self.tree.")
    
    if curr_sector not in self.tree[curr_world]:
        raise KeyError(f"Sector '{curr_sector}' not found in world '{curr_world}' in self.tree.")
    
    arenas = list(self.tree[curr_world][curr_sector].keys())
    return ", ".join(arenas)
